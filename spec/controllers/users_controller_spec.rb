describe UsersController, type: :controller do
  ...
  describe 'GET #raffle' do
    it 'should send the notify email' do
      get :raffle
      expect(ActionMailer::Base.deliveries.all? {|m| m.subject == "Atenção!!! Veja quem será seu Amigo-X!"}).to be true
    end

    it 'should send the notify2 email' do
      get :raffle
      delete :destroy, {:id => many_users.first.id}
      ActionMailer::Base.deliveries = []
      get :raffle
      expect(ActionMailer::Base.deliveries.all? {|m| m.subject == "Atenção!!! Veja quem será seu novo Amigo-X!"}).to be true
    end
  end
